...
def obre_imatgeRGB(ruta_imatge, mida=None):
    """
        Nota: ens hem d'assegurar que les imatges estiguin dins l'espectre de color RGB
        ja que es l'espectre que es necesita per fer la FFT.
    """
...
def calcula_MagnitudFase(img_rgb):
    """
        Nota: aquest mètode ens serveix per convertir la imatge a escala de grisos,
        despres aplicar la FFT i seguidament realitzar el logaritme adamunt el valor absolut
        del resultat de la FFT. 
        Tensor RGB -> (3,H,W) //tres canals, llargaria, amplada.
        Tensor Gris -> (1,H,W) // un canal, es a dir, una intensitat per pixel, exactament el que necessitam per la FFT.
    """
    ...
    #Nota: emrpam abs i angle per pasar el nombre complex a forma polar
...
def calcula_lbp(img_gris, punts=8, radi=1, metode="uniform"):
    """
        Nota: el metode LBP te com objectiu construir una nova representacio de la imatge
        basada en patrons locals de textures.
    """
